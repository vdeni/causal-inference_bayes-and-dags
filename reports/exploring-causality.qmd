...
in this model, as there are no arrows entering $X$. To obtain
$\text{E}(Y | do(X = 6), Z)$, we have

\begin{align}
    \text{E}(Y | do(X = 6), Z) &= \text{E}(Y | X = 6, Z) \\
        &= (\beta_{YX} \times 6 + \beta_{YZ} \times Z),\; \text{where} \\
        Z &= f(X = 6) = \text{normal}(\beta_{ZX} \times 6, 1).
\end{align}

According to Pearl's framework, as the $X$ variable has no parents,
there's no need to adjust for anything if we want to obtain the total causal
effect of $X$ on $Y$. Therefore, for $\text{E}(Y | do(X = 6), Z)$ we have:
...
Z_distr_do = Distributions.Normal(b_zx * 6, 1)

Z_do = Random.rand(Z_distr_do,
                   1000)

mean(b_yx * 6 .+ b_yz * Z_do)
```
