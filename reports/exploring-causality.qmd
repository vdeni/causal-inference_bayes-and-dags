...
converting the $do(X = 6)$ into $X = 6$. We're allowed to do this if $X$ is
independent of $Y$, conditional on $Z$, in a modified DAG where all arrows
exiting $X$ are removed. This may be written as
$(Y \perp X | Z)_{G_{\underline{X}}}$, where $G_{\underline{X}}$ stands for
"graph where all arrows going out of $X$ are removed". This graph is shown in
Figure @fig-three-var-confounding-x-out-mutil.
...
between $X$ and $Y$. We do that by integrating over the values of $Z$, which
leads to the same set of equations we've had when looking at the expectation
