...
which could have been expected, given that we've estimated both $b{XY}$ and
$b{ZY}$ approximately correctly. So we basically just reran the previous
calculation.
...
estimates of $b{XY}$ and $b{ZY}$, simulate a
...
$$
    \text{E}(Z') = \mu_{z'_i} = b{XZ} \times x_i,\; \text{where}\;
        x_i = 6\; \forall\, i
$$
...
\begin{align}
    \text{E}(Y | X = x, Z = z) &= \mu_{y_i} \\
        &= b{XY} x_i + b{ZY} z_i \\
    p(Z = z | X = 0) = normal(Z = z | \mu_{z_i}, \sigma_Z) \\
        &= normal(Z = z | b_{XZ} x_i, \sigma_Z)
\end{align}
