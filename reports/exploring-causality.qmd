...
removed". This graph is shown in Figure @fig-three-var-confounding-x-out-mutil.
...
\begin{align}
    \text{E}(Y | do(X = 6), Z) &= \text{E}(Y | X = 6, Z) \\
        ...
        &= \int_{z \in Z}\text{E}(Y | X = 6, z) f_Z(z)\,
            \text{d}z \\
        &= \int_{z \in Z}(\hat{\beta}_{YX} \times 6 +
            \hat{\beta}_{YZ} \times z)f_Z(z)\, \text{d}z.
\end{align}

```{julia}
#| echo: false
Markdown.parse("""Let's take the posterior means of the parameter estimates
               for \$\\hat{\\beta}_{XY}\$ and \$\\hat{\\beta}_{ZY}\$, that is
               \$\\beta_{XY} =
...
which could have been expected, given that we've estimated both $\beta_{XY}$ and
$\beta_{ZY}$ approximately correctly. So we basically just reran the previous
calculation.
...
\begin{align}
    \text{E}(Y | do(X = 6), Z') &= \text{E}(Y | X = 6, Z') \\
        &= \beta_{XY} \times 6 + \beta_{ZY} \times Z'.
\end{align}

To obtain an estimate of $\text{E}(Y | do(X = 6), Z)$ we can take our posterior
estimates of $\beta_{XY}$ and $\beta_{ZY}$, simulate a
set of $Z'$ values, and calculate

$$
    \frac{1}{N} \sum_{i}^N (\hat{\beta}_{XY} \times 6 +
        \hat{\beta}_{ZY} \times z'_i)
$$
...
\begin{align}
    \text{E}(Y | X = 6, Z') &= \frac{1}{N} \sum_{i}^N (\hat{\beta}_{XY} \times 6
        + \hat{\beta}_{ZY} \times z'_i) \\
    &= \hat{\beta}_{XY} \times 6 + \hat{\beta}_{ZY} \times
...
$$
    \text{E}(Z') = \mu_{z'_i} = \beta_{XZ} \times x_i,\; \text{where}\;
        x_i = 6\; \forall\, i
$$
